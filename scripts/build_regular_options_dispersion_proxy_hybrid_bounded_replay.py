...
def _candidate_blockers(row: dict[str, Any]) -> list[str]:
    if row.get("candidate_selected") is False:
        return [str(item) for item in _as_list(row.get("blockers"))] or ["no_pair_candidate_selected"]
...
def _resolve_candidate(row: dict[str, Any], conn: sqlite3.Connection | None) -> dict[str, Any]:
    if row.get("candidate_selected") is False:
        ...
        return {
            ...
            "entry_date_et": row.get("entry_date_et"),
            "entry_minute_et": row.get("entry_minute_et"),
            "exit_date_et": row.get("exit_date_et"),
            "exit_minute_et": row.get("exit_minute_et"),
            "index_debit_long_contract": row.get("index_debit_long_contract"),
            "index_debit_short_contract": row.get("index_debit_short_contract"),
            "constituent_credit_short_contract": row.get("constituent_credit_short_contract"),
            "constituent_credit_long_contract": row.get("constituent_credit_long_contract"),
            "denominator_status": row.get("denominator_status") or "no_pair_candidate_selected",
            "blockers": blockers,
            "pricing_blockers": blockers,
            "proof_blockers": [],
            "side_aware_entry_quote_status": "not_applicable_no_pair_candidate",
            "side_aware_exit_quote_status": "not_applicable_no_pair_candidate",
            "side_aware_quotes_resolved": False,
            "pair_entry_cashflow": None,
            "pair_exit_value": None,
            "fees_usd": ROUND_TRIP_CONTRACT_SIDES * PER_CONTRACT_FEE_USD,
            "net_pnl_usd": None,
            "pair_max_loss_usd": _safe_float(row.get("pair_max_loss_usd")),
            "required_collateral_usd": _safe_float(row.get("required_collateral_usd")),
            "priced_exact": False,
