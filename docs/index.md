...
- `docs/regular-options-point-in-time-dispersion-concentration-proxy.md`
  - generated read-only point-in-time dispersion/concentration proxy materializer; current Alpaca-backed generated source pass reports available point-in-time proxy rows for the dispersion branch
- `docs/regular-options-dispersion-proxy-hybrid-replay-readiness.md`
  - generated read-only replay-readiness audit for `index_constituent_dispersion_proxy_defined_risk_hybrid_v1`, currently ready after clearing VIX, point-in-time dispersion/concentration inputs, pair construction, all-leg side-aware pricing, max-loss/collateral, denominator mapping, strict-new dedupe, CVX scope, holdout guard, and proof-boundary prerequisites without running replay
- `docs/regular-options-dispersion-proxy-hybrid-bounded-replay.md`
  - generated read-only bounded replay harness for `index_constituent_dispersion_proxy_defined_risk_hybrid_v1`; current real-artifact check has `494` proxy denominator rows and remains blocked on missing pair candidate rows, with no quote import, evidence mutation, scanner change, forward-proof claim, or promotion
