...
- The momentum-continuation research branch currently has 264 historical exact rows, PF 2.8554, and a conservative clustered-bootstrap PF lower bound of 1.71. It remains non-proof and blocked: synchronized trusted quote coverage is 270/353 (76.49%) with 83 exact exit quote repairs, no preregistered stress test exists, and no historical row counts toward forward proof. Raw `missing_quote_date` values are preserved as planned policy exits rather than misclassified as absent lifecycle evidence.
