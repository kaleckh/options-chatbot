...
- Historical, forward, fresh-window, and VRP proof paths now fail closed on late/duplicate point-in-time inputs, missing entry evidence, non-causal surfaces, incomplete lifecycle/policy/contract continuity, unbound manifest corpora, unproved chain completeness, unknown close metadata, zero-trade readiness, and missing non-self-asserted forward entry quote-store verification. Forward diagnostic capture may continue, but proof-valid completion counting and evaluation remain hard-blocked by `entry_quote_store_verification_not_established` until an exact content verifier exists. These controls do not create positive evidence; strict forward remains 0/30 and the paper shortlist remains empty.
