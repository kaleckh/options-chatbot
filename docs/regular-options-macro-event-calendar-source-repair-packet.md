...
- `macro_event_long_strangle`: event blockers `['macro_event_calendar_source_missing']`; remaining non-event blockers `['point_in_time_vix_source_missing', 'missing_vix_bucket_threshold_policy', 'vix_bucket_date_coverage_incomplete']`
- `post_event_iv_crush_iron_condor`: event blockers `['future_replay_requires_point_in_time_macro_event_calendar']`; remaining non-event blockers `['point_in_time_vix_source_missing', 'iv_event_premium_proxy_missing']`
- `direct_vix_source_repair`: event blockers `[]`; remaining non-event blockers `['direct_vix_source_import_materialization_pending']`
...
future_post_event_iv_crush_readiness_audit_not_implemented_yet
```
