...
3. Resume frozen-cohort fresh-evidence collection until at least one frozen lane reaches `30` post-freeze exact realized P&L rows, or keep the named-gate defect report current if the funnel remains blocked after 10 trading sessions.
...
7. Phase 2 is frozen. Operate the preregistered cohort only: `volatility_expansion_observation` plus the clean `bullish_pullback_observation` carrier set (`IWM`, `AAPL`, `GOOGL`, `UNH`, `LLY`, `JNJ`, `XOM`, `CVX`, `COP`, `NEM`), freeze date `2026-06-14`, eval date `2026-07-28`. Run `npm run daily-ops`, keep non-cohort regular lanes parked, and collect fresh exact realized P&L rows after freeze without policy edits or protected-holdout consumption.
