...
Do not repeat the same NEM, LLY, AAPL, or UNH exact-date repair loop unless a new provider/source can produce positive bid/ask rows for the still-missing dates. The next repair attempt should either target a different high-priority row with untested exact-date rows or improve the importer/reporting path with a de-duplicated repair manifest, repair-target freshness checks, and true no-write mode.

Subagent debate consensus for the next high-priority checks before more imports: classify the latest remaining exact gaps, run symbol-sleeve and profit-capture queue no-write readbacks, run the operating scorecard no-write readback, run proof/scanner contract regressions, run Trading Desk operator-risk audits, validate pending scan candidates only during a fresh executable quote window, and keep AI commodity proof-lane checks separate from regular-options ThetaData repair.
