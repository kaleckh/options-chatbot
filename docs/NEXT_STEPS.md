...
- latest gateboard data/profit split: repository integrity is clean (`0` hard violations, `0` diagnostics), candidate lifecycle is visible (`34` fresh candidates), but fresh exact realized P&L is `0`, promotion-ready rows are `0`, broad untracked missed-pick economics are `206` rows / `70` winners / `136` losers / PF `0.34` / avg `-15.28%`, lane state is `13` diagnostic / `1` paper-probation / `0` live-validation / `0` auto-track, open-risk governor is blocked by live-exact QQQ `id=537`, suggested-trade review has attention on `id=138`, and paper-shortlist eligible count is `0`.
- latest candidate outcome ledger status is `ledger_live_entry_blocked_collect_evidence` with `106` ledger rows: `1` open-risk governor blocker, `1` open-position executable-review refresh, `1` suggested-trade review refresh, `1` exact-exit evidence row for QQQ `id=537`, `5` paper-review create/link rows, `8` paper/probation exact-entry rows, `4` missing fill-attempt evidence rows, `16` no-longer-matched/archive rows, `21` Tier A fresh-bridge waits, `39` historical repair rows, and `9` guardrail/lane-mismatch rows. It is read-only and has `live_policy_change=false`.
...
- latest open-risk resolution plan is `open_risk_resolution_plan_ready_blocked_for_market_window`: `2` row-specific review actions, live-exact QQQ `id=537` negative open risk, display-only SELL SBUX `id=104`, `12` open regular rows, `10` negative rows, avg open P&L `-44.14%`, median `-47.58%`, and `live_policy_change=false`. It is generated by `npm run options:plan:open-risk-resolution` at `docs/regular-options-open-risk-resolution-plan.md` and `data/forward-tracking/regular_options_open_risk_resolution_plan_latest.json`; it replaces generic open-risk queue items but does not submit orders or auto-close display-only marks.
...
- latest risk-budget sizing replay is `sizing_replay_built_open_risk_blocked`: `206` priced untracked source rows, baseline one-contract net `-$16,314.00` at PF `0.34`, best research scenario `paper_shadow_only` net `+$972.30` at PF `1.84`, and tiered paper-shadow/full plus retest-quarter net `+$355.19` at PF `1.16`. Promotion remains false because open risk is blocked, rows are historical research/backfill rather than production sizing proof, and fresh exact realized sizing evidence is required. It is generated by `npm run options:replay:risk-budget-sizing` at `docs/regular-options-risk-budget-sizing-replay.md` and `data/forward-tracking/regular_options_risk_budget_sizing_replay_latest.json`, with `live_policy_change=false`.
...
- latest monthly all-lanes profitability audit is `profitability_iteration_ready_blocked_for_promotion`: baseline missed-pick economics are `206` rows, PF `0.34`, avg `-15.28%`; recent month `2026-05` is `paper_only_recent_break`; execution realism is `ready` with no execution blockers. Top/contract execution-alternative replay has `12` true top-spread rows and `12` true contract-replacement rows, minute-exit has `12` true minute-exit P&L rows, structure-specific replay has `12` true read-only structure P&L rows, and exact realized P&L rows remain `0`. Open risk is `open_risk_governor_blocked`, promotion is blocked with `25` blockers, oracle ceiling remains `not_available_replay_gap`, `0` / `10` candidate rules are paper candidates, `10` are rejected/overfit, `10` are archived, and `0` rejected rules remain unarchived. Lane dispositions are classified for all `13` active regular supervised lanes: `1` `paper_shadow`, `4` `quarantine`, `3` `retest`, `5` `needs_replay_engine`, `0` `profitable_candidate`, and `0` `archive`; all `4` quarantine lanes are read-only archived. The execution-alternative and minute-exit quote-import actions are gone after exact quote consumption, and the next-evidence queue is now `11` actions. Top queue items remain `execute_open_risk_resolution_review_plan`, `execute_suggested_trade_review_plan`, and `collect_exact_exit_evidence`. It is generated by `npm run options:audit:monthly-profitability` at `docs/monthly-all-lanes-profitability-audit.md` and `data/forward-tracking/monthly_all_lanes_profitability_audit_latest.json`, with `live_policy_change=false`.
...
Current read: `paper_probation_blocked`, `6` legacy pre-promotion volatility rows, `0` current paper exact pending rows, `0` promotion-ready rows excluding legacy, and `open_risk_governor_blocked` on live-exact QQQ `id=537`.
...
- the missed-pick outcome audit now gates regular lane promotion. `scripts/audit_missed_regular_picks_outcomes.py` priced `210` raw missed rows with trusted intraday exact-contract marks through `2026-06-04`, found `4` tracked rows with stored P&L and `206` untracked rows with `70` winners / `136` losers, avg net `-15.28%`, median `-11.5%`, profit factor `0.34`, and `-$16,314.00` one-spread net dollars. `scripts/analyze_missed_regular_picks_failure_modes.py` confirms the read is `clean_for_failure_analysis`, not a remaining quote/P&L blocker, and classifies the result as `data_clean_strategy_unprofitable`: `37` rows were `<= -50%`, `15` were `<= -80%`, seven lanes remain diagnostic-only, XLK/SPY/TSLA/IWM are the largest damage clusters, and debit `>=45%` of width plus DTE `>=36` are retest-required diagnostic guardrail candidates. `scripts/analyze_missed_regular_picks_filter_matrix.py` adds the frozen counterfactual matrix: only `volatility_expansion_observation` passes lane promotion (`24` rows, PF `1.72`, avg `+6.75%`), and lane gate plus self-guardrails keeps `10` rows at PF `84.9`, avg `+34.87%`, and later-date split pass, but loses `61` historical winners. `scripts/lane_promotion_state.py` now turns those reads plus fresh evidence, open risk, and circuit-breaker state into the shared promotion artifact: current state is `13` diagnostic lanes, `1` paper/probation lane, `0` live-validation lanes, `0` auto-track lanes, and `1` unresolved current live-exact negative open row. Passed lanes therefore route to paper/probation, not live validation. The daily all-lanes safety net loads a fresh gate and fresh promotion-state report before queueing and routes clear candidates to diagnostics or paper/probation if either report is missing, malformed, future-dated, missing `generated_at_utc`, stale, has unpriced rows, has incomplete tracked P&L, lacks lane rows, lacks walk-forward/fresh-paper depth, has unresolved live-exact negative risk, or is under a lane-specific circuit breaker; exact duplicate spreads selected by multiple lanes are suppressed to one risk owner. Pending validation rechecks the same gates before any auto-track rerun, writes paper-only rows for blocked/probation candidates, and fails closed if either report is unusable. Scanner validation reruns enforce them with `OPTIONS_ENFORCE_LANE_PROFITABILITY_GATE=1`.
