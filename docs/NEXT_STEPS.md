...
- trusted `thetadata_opra_nbbo_1m` intraday coverage for the 13-symbol proof/import set (`SPY`, `QQQ`, `IWM`, `AAPL`, `GOOGL`, `UNH`, `LLY`, `JNJ`, `XOM`, `CVX`, `COP`, `NEM`, `DIA`) is now `305` shared dates from `2024-05-22` through `2026-06-04`; target for a two-year surface is `504` shared dates, so `199` shared dates remain.
...
- batches `2130` through `2134` imported `2024-05-22` through `2024-07-26` for the 13-symbol set with `1,030,792` trusted intraday rows, `0` duplicates, and `0` rejects.
...
1. Continue old-history imports in chunks until the 13-symbol proof/import set reaches the two-year target or ThetaData returns a provider/license blocker. Next chunk starts at `2024-07-29`. Use the same parameters as batches `2130` through `2134`: `--snapshot-kind intraday --start-time 09:45 --end-time 15:55 --interval 1h --min-dte 21 --max-dte 60 --strike-range 20 --right both --source thetadata_opra_nbbo_1m`.
