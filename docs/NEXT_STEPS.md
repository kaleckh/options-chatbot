...
- trusted `thetadata_opra_nbbo_1m` intraday coverage for the 13-symbol proof/import set (`SPY`, `QQQ`, `IWM`, `AAPL`, `GOOGL`, `UNH`, `LLY`, `JNJ`, `XOM`, `CVX`, `COP`, `NEM`, `DIA`) is now `453` shared dates from `2024-05-22` through `2026-06-04`; target for a two-year surface is `504` shared dates, so `51` shared dates remain.
- paid-data readiness is `not_ready` after batch `2143` because `CVX` executable quote coverage is `89.23%`, below the `90%` floor; do not use the 13-symbol surface for a nomination until this clears or the lane explicitly excludes/fails the affected symbol under a preregistered rule.
...
- batches `2130` through `2143` imported `2024-05-22` through `2025-02-28` for the 13-symbol set with `4,582,228` trusted intraday rows, `0` duplicates, and `0` rejects.
- `docs/regular-options-feature-store.md` is now the point-in-time feature-store readback: `10,926,420` trusted intraday rows, all `13` symbols available, `453` shared quote dates, and joins require `feature.tradable_after_time <= candidate_entry_time`.
...
1. Continue old-history imports in chunks until the 13-symbol proof/import set reaches the two-year target and the low-executable `CVX` audit clears, or ThetaData returns a provider/license blocker. Next chunk starts at `2025-03-03`. Use the same parameters as batches `2130` through `2143`: `--snapshot-kind intraday --start-time 09:45 --end-time 15:55 --interval 1h --min-dte 21 --max-dte 60 --strike-range 20 --right both --source thetadata_opra_nbbo_1m`.
...
3. Build or repair the missing inputs named by the robust-search report before treating any historical result as a nomination: feature-store depth to at least `504` shared trusted intraday dates, baseline ablation after costs, VIX/regime context coverage, and enough newest holdout rows to reach at least `30` exact final-holdout trades.
4. Rerun `npm run options:features:regular-options` and `npm run options:robust-search:regular-options` after each historical import chunk or source-replay repair.
...
- open risk is blocked again because QQQ `id=537` is a negative live-exact row whose latest executable review is now stale. `scripts/build_regular_options_open_risk_resolution_plan.py` reports `open_risk_resolution_plan_ready_blocked_for_market_window` with one action: refresh the live-exact negative open-position review during a valid market-data window. Do not close a HOLD row to manufacture realized evidence.
