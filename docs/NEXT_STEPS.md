...
- repair-attempt readback is now available. `scripts/build_regular_options_repair_attempt_readback.py` turns importer summaries into latest keyed attempts and `docs/regular-options-repair-attempts.md`. Current baseline scanned `156` old ThetaData exact-missing summaries and has `0` keyed latest attempts because those summaries predate the new ledger. After any new importer summary is written, rebuild the readback and then rebuild the profit-capture queue so `repair_actionability` can show `imported_pending_replay`, `lookahead_only_not_exact_proof`, or `current_source_exhausted` instead of asking future agents to remember it from chat.
...
The next useful work after that is a strict Tier A fresh-match bridge: only `paper_review_candidate` rows can enter a paper shortlist, and only after a fresh executable quote-window scanner match. Treat Tier B as `watch_repair_only`, Tier C as `historical_signature_only`, and GOOGL as blocked from chase/promotion while unresolved or zero-bid issues remain.

Do not repeat the same NEM, LLY, AAPL, UNH, or WMT exact-date repair loop unless a new provider/source can produce positive bid/ask rows for the still-missing dates. Before any next import, run the importer with `--plan-only --json` plus the narrowest useful `--ticker`, `--contract-symbol`, or `--quote-date` filters to inspect the de-duplicated target manifest and source occurrences, then use `--dry-run --json` if a provider fetch is needed before writing CSVs or importing rows. A lookahead-only result is useful memory but not proof repair. The next repair attempt should target a different high-priority row with untested exact-date rows or a new source that can answer the classified no-match contracts.
