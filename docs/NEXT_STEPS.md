...
Do not repeat the same NEM, LLY, AAPL, or UNH exact-date repair loop unless a new provider/source can produce positive bid/ask rows for the still-missing dates. Before any next import, run the importer with `--plan-only --json` plus the narrowest useful `--ticker`, `--contract-symbol`, or `--quote-date` filters to inspect the de-duplicated target manifest and source occurrences, then use `--dry-run --json` if a provider fetch is needed before writing CSVs or importing rows. The next repair attempt should target a different high-priority row with untested exact-date rows or a new source that can answer the classified no-match contracts.
