...
`equity-min-direction-score-45`

## Hypothesis

Tightening equity `entry.min_direction_score` to `45.0` may improve replay quality under the fixed matrix without changing fills, watch/block policy, or stability rules.

## Exact Rule Change

- Rule: `STRATEGY_PROFILES["equity"]["entry"]["min_direction_score"]` to `45.0`.
...
- `python scripts/autoresearch_cycle.py --slug equity-min-direction-score-45 --proposal docs/autoresearch/proposals/sixty-run-grid/equity-min-direction-score-45.md --playbook broad --playbook bullish_momentum --playbook bearish_defensive --compare-to <baseline decomposition run dir>`
