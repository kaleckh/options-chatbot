...
`equity-min-direction-score-86`

## Hypothesis

Tightening equity `entry.min_direction_score` to `86.0` may improve replay quality under the fixed matrix without changing fills, watch/block policy, or stability rules.

## Exact Rule Change

- Rule: `STRATEGY_PROFILES["equity"]["entry"]["min_direction_score"]` to `86.0`.
...
- `python scripts/autoresearch_cycle.py --slug equity-min-direction-score-86 --proposal docs/autoresearch/proposals/sixty-run-grid/equity-min-direction-score-86.md --playbook broad --playbook bullish_momentum --playbook bearish_defensive --compare-to <baseline decomposition run dir>`
