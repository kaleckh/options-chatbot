...
`equity-min-tech-score-82`

## Hypothesis

Tightening equity `entry.min_tech_score` to `82.0` may improve replay quality under the fixed matrix without changing fills, watch/block policy, or stability rules.

## Exact Rule Change

- Rule: `STRATEGY_PROFILES["equity"]["entry"]["min_tech_score"]` to `82.0`.
...
- `python scripts/autoresearch_cycle.py --slug equity-min-tech-score-82 --proposal docs/autoresearch/proposals/sixty-run-grid/equity-min-tech-score-82.md --playbook broad --playbook bullish_momentum --playbook bearish_defensive --compare-to <baseline decomposition run dir>`
