...
`equity-min-direction-score-75`

## Hypothesis

Tightening equity `entry.min_direction_score` to `75.0` may improve replay quality under the fixed matrix without changing fills, watch/block policy, or stability rules.
...
- `python scripts/autoresearch_cycle.py --slug equity-min-direction-score-75 --proposal docs/autoresearch/proposals/sixty-run-grid/equity-min-direction-score-75.md --playbook broad --playbook bullish_momentum --playbook bearish_defensive --compare-to <baseline decomposition run dir>`
