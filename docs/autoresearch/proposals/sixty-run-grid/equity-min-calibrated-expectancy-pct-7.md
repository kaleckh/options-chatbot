...
`equity-min-calibrated-expectancy-pct-7`

## Hypothesis

Tightening equity `filters.min_calibrated_expectancy_pct` to `7.0` may improve replay quality under the fixed matrix without changing fills, watch/block policy, or stability rules.
...
- `python scripts/autoresearch_cycle.py --slug equity-min-calibrated-expectancy-pct-7 --proposal docs/autoresearch/proposals/sixty-run-grid/equity-min-calibrated-expectancy-pct-7.md --playbook broad --playbook bullish_momentum --playbook bearish_defensive --compare-to <baseline decomposition run dir>`
