...
As of the 2026-06-07 monthly readback:
...
- execution realism: `blocked_replay_gaps`
- minute-exit quote import plan: `minute_exit_quote_import_plan_ready_engine_blocked`, `24` parsed demands, `0` unparsed demands, `8` grouped ThetaData command groups, quote dates `2026-05-21`, `2026-05-29`, `2026-06-04`, and `2026-06-05`, and underlyings `IWM`, `QQQ`, and `SPY`; true minute-exit P&L remains `0` until quotes and a replay engine exist
- execution-alternative coverage: `12` top-spread candidates, `12` contract-replacement candidates, `0` true alternative replay P&L rows, entry quote coverage missing, exit quote coverage partial, and `58` unique quote-demand targets (`33` entry, `25` exit) for the next exact OPRA/NBBO import/query pass
- execution-alternative quote import plan: `execution_alternative_quote_import_plan_ready`, `58` parsed demands, `0` unparsed demands, `8` grouped ThetaData command groups, quote dates `2026-05-21`, `2026-05-29`, `2026-06-04`, and `2026-06-05`, and underlyings `IWM`, `QQQ`, and `SPY`
...
- promotion: blocked with `29` blockers
...
- next-evidence queue: `18` actions after replacing duplicate generic open-risk actions with `execute_open_risk_resolution_review_plan`, generic top-spread/contract-replacement replay-build actions with `run_execution_alternative_quote_import_commands`, and generic minute-exit replay build work with `run_minute_exit_quote_import_plan_commands`
...
2. `collect_exact_exit_evidence`: convert missing exact realized P&L into trusted readbacks.
3. `collect_fresh_paper_rows`: add fresh exact entry/exit evidence for paper/probation lanes.
4. `run_execution_alternative_quote_import_commands`: import/query the exact trusted quotes needed before claiming top-spread or contract-replacement replay P&L.
5. `run_minute_exit_quote_import_plan_commands`: import/query the exact trusted minute quotes needed before claiming minute-exit replay P&L or discussing stop policy.
6. `build_minute_exit_replay`: replace daily close-check assumptions with minute-level exact OPRA/NBBO exit replay after trusted minute quote coverage exists.
7. `build_contract_replacement_replay`: test whether alternate contracts survive exit liquidity better.
8. `build_top_spread_alternative_replay`: replay top alternatives and liquidity-first selection without changing live selection policy.
9. `build_risk_budget_sizing_replay`: prove sizing from exact evidence and open-risk history before changing size tiers.
10. `build_lane_outcome_replay`: separate no-signal, no-exact-candidate, unselected, and unpriced lane blockers before making lane profitability claims.
11. `build_event_data_spine`: add event/earnings/vol-crush context before event-sensitive lane changes.
12. `build_or_repair_lane_scan_hypothesis_before_pnl_replay`: repair no-signal lane coverage before treating missing outcome lanes as unprofitable.
13. `archive_overfit_rule`: explicitly retire high-PF tiny or winner-damaging rules.
14. `retest_filter`: freeze entry-time-only candidate rules and rerun later-date/point-in-time/fresh-paper validation.
...
```powershell
uv run --locked python -m py_compile scripts\build_monthly_all_lanes_profitability_audit.py tests\test_monthly_all_lanes_profitability_audit.py
