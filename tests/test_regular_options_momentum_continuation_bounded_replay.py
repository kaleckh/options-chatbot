...
def _source_replay(tmp: Path, *, valid: bool = True) -> Path:
    ...
    _write_json(
        path,
        {
            ...
            "diagnostic_only_existing_marks": {"metrics": {"net_pnl_usd": -10.0, "profit_factor": 0.8}},
        },
    )
    return path
...
def _resolution(tmp: Path, *, candidate: bool = True) -> Path:
    ...
    _write_json(
        path,
        {
            ...
            "status": "momentum_continuation_proof_candidate_for_review_not_forward_proof" if candidate else "momentum_continuation_blocked_missing_local_proof_inputs",
            "concept_id": replay.CONCEPT_ID,
            "accepted_profitability": False,
            "historical_rows_are_forward_proof": False,
            "proof_qualified_rows_after_resolution": 0 if not candidate else 30,
            "blockers": [] if candidate else ["missing_point_in_time_vix_bucket", "missing_point_in_time_breadth_confirmation"],
            "resolution_counts": {
                "side_aware_quotes_resolved": 1,
                "point_in_time_inputs_resolved": 0 if not candidate else 30,
                "blocker_counts": {} if candidate else {"missing_point_in_time_vix_bucket": 2},
            },
            "strict_research_metrics": {"net_pnl_usd": None, "profit_factor": None, "bootstrap_pf_lower_bound_5pct": None, "stress_pf": None},
            "side_aware_diagnostic_metrics": {"net_pnl_usd": 100.0, "profit_factor": 2.0},
        },
    )
    return path
...
class MomentumContinuationBoundedReplayTests(unittest.TestCase):
    ...
    def test_fixture_replay_uses_side_aware_debit_spread_formula_and_statuses(self) -> None:
        with WorkspaceTempDir(prefix="momentum-bounded") as tmp_dir:
            ...
            rows = [
                ...
                _candidate(ticker="SPY", entry_date="2026-05-22", breadth_confirmed=False),
        ...
        self.assertEqual(report["status"], "rejected_momentum_continuation_bounded_replay")
    ...
    def test_write_outputs_writes_latest_and_docs(self) -> None:
        with WorkspaceTempDir(prefix="momentum-bounded") as tmp_dir:
            tmp = Path(tmp_dir)
            report = replay.build_report(**self._paths(tmp, resolution_candidate=False))
            artifacts = replay.write_outputs(report, output_dir=tmp / "out", docs_report=tmp / "docs" / "report.md")
