...
def _insert_surface(conn: sqlite3.Connection, symbol: str, quote_date: str, expiry: str) -> None:
    rows = [
    ...
    for contract, strike, bid, ask in rows:
        conn.execute(
            ...
            values (?, ?, 640, ?, ?, ?, 'put', ?, ?, ?, 1)
            """,
            (f"{quote_date}T14:40:00Z", quote_date, symbol, contract, expiry, strike, bid, ask),
        )


class RegularOptionsVrpCreditSpreadQuoteSurfaceTests(unittest.TestCase):
    def test_ready_when_all_symbols_have_required_month_and_latest_coverage(self) -> None:
    ...
    def test_blocks_when_one_symbol_lacks_required_months(self) -> None:
        ...
        self.assertEqual(report["blockers"], ["missing_index_credit_spread_quote_surface"])
