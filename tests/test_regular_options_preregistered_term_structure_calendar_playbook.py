...
from scripts import build_regular_options_preregistered_term_structure_calendar_playbook as playbook
...
class PreregisteredTermStructureCalendarPlaybookTests(unittest.TestCase):
    ...
    def test_report_is_design_only_and_read_only(self) -> None:
        with WorkspaceTempDir(prefix="term-calendar-playbook") as tmp_dir:
            tmp = Path(tmp_dir)
    ...
    def test_concept_matches_gpt55_selected_design(self) -> None:
        ...
        self.assertEqual(concept["structure"], "defined_risk_calendar_or_diagonal_debit_spreads_only")
    ...
    def test_side_aware_formulas_and_denominator_statuses_are_registered(self) -> None:
        ...
        self.assertEqual(formulas["entry_debit"], "long_back_month_ask - short_front_month_bid")
        self.assertEqual(formulas["exit_debit_or_value"], "long_back_month_bid - short_front_month_ask")
    ...
    def test_future_requirements_include_calendar_specific_risks(self) -> None:
        with WorkspaceTempDir(prefix="term-calendar-playbook") as tmp_dir:
            tmp = Path(tmp_dir)
            report = playbook.build_report(oracle_packet_path=self._packet_fixture(tmp))
        ...
        self.assertIn("calendar/diagonal side-aware bid/ask entry pricing", requirements)
        self.assertIn("front-leg expiry or roll handling", requirements)
        self.assertIn("short-leg assignment and expiration classification", requirements)
        self.assertIn("point-in-time VIX and term-structure inputs", requirements)
        self.assertIn("future IV", leakage)
    ...
    def test_write_outputs_writes_docs_and_latest(self) -> None:
        with WorkspaceTempDir(prefix="term-calendar-playbook") as tmp_dir:
            ...
            self.assertIn("Regular Options Preregistered Term Structure Calendar Playbook", markdown)
